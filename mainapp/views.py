...
def patient_viewdoctors(request):
    if request.method == 'POST':
        doctor_name = request.POST.get('search_query')
        # Check if the entered doctor name matches any doctor's full name
        if Doctors.objects.filter(full_name=doctor_name).exists():
            # Redirect to view_doctors page
            return redirect('view_doctors')
        else:
            # Display message indicating incorrect doctor name
            messages.error(request, 'Please insert correct doctor name.')
            return render(request, 'main/patient-viewdoctors.html')
    else:
        return render(request, 'main/patient-viewdoctors.html')
